    ...
    if (this.state.hasError) {
      return (
        ...
        >
          ...
          <h2 style={{ fontSize: "1.2rem", color: "#fff", marginTop: "1rem" }}>
            STACK_TRACE:
          </h2>
          <pre
            style={{
...
const BOARD_MEMBERS = [
  ...
  },
  {
    ...
    name: "Yashi Jain",
    role: "UI/UX Lead",
    image: "/devTeam/Yashi.webp",
    linkedin:
      "https://www.linkedin.com/in/yashi-jain-511a14372?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=ios_app",
...
const MARQUEE_TEXT_1 =
  "CODE BLUE ◆ INNOVATION ◆ IEEE SBNU ◆ TECHNICAL EXCELLENCE ◆ FULL STACK DEVELOPMENT ◆ PIXEL PERFECT ◆ CLOUD NATIVE ◆ SCALABLE SYSTEMS";
const MARQUEE_TEXT_2 =
  "HIGH PERFORMANCE ◆ REACT & NEXT.JS ◆ CODE BLUE ◆ DEPLOYING THE FUTURE ◆ ELITE ENGINEERING ◆ NEXT LEVEL ◆ UNMATCHED QUALITY";
...
function CodeBlueContent() {
  ...
  useEffect(() => {
    ...
    // CODE from Left
    tl.fromTo(
      ".cb-proximity-title:not(.cb-proximity-title-blue)",
    ...
    tl.to(
      ...
      {
        textShadow:
          "0 0 50px rgba(255, 255, 255, 0.8), 0 0 100px rgba(94, 184, 255, 0.8)",
        filter: "brightness(1.5)",
        scale: 1.02,
        duration: 0.05,
        ease: "power2.in",
        onComplete: () => {
          gsap.to(".cb-hero-title-wrap", {
            x: -5,
            duration: 0.05,
            yoyo: true,
            repeat: 3,
  ...
  useEffect(() => {
    ...
    gsap.set(".cb-proximity-title", {
      opacity: 1,
      x: 0,
      scaleX: 1,
      filter: "blur(0px)",
    });
    gsap.set(".cb-proximity-title-blue", {
      opacity: 1,
      x: 0,
      scaleX: 1,
      filter: "blur(0px)",
    ...
    const ctx = gsap.context(() => {
      ...
      ScrollTrigger.create({
        ...
        start: "top 80%",
        onEnter: () => {
          gsap.to(attitudeTextRef.current, {
            opacity: 1,
            y: 0,
            duration: 1,
            ease: "expo.out",
          });
          gsap.to(attitudeSubRef.current, {
            opacity: 1,
            duration: 0.6,
            delay: 0.4,
          });
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          gsap.to(outroTitleRef.current, {
            opacity: 1,
            duration: 0.8,
            delay: 0.3,
            ease: "expo.out",
          });
          gsap.to(outroEndRef.current, {
            opacity: 1,
            duration: 0.5,
            delay: 0.7,
          });
      ...
      .map((_, i) => (
        <span key={i} className="cb-marquee-item">
          {text.split(" ").map((word, j) => (
            <span key={j}>
              {word === "◆" ? <span className="dot">◆</span> : word + " "}
            </span>
          ))}
        </span>
      ));
  ...
  return (
    <div
      className={`cb-page ${introDone ? "cb-boot-complete" : ""}`}
      ref={pageRef}
    >
      ...
      <div className="cb-line-numbers">
        {Array(50)
          .fill(0)
          .map((_, i) => (
            <div key={i} className="cb-line-number">
      ...
      {/* ═══════ HERO ═══════ */}
      <section
        className="cb-hero-section"
        ref={heroRef}
        style={{ justifyContent: "center" }}
      >
        {/* CINEMATIC BARS */}
        <div
          className="cb-cinema-bar cb-cinema-bar-top"
          ref={(el) => {
            if (el) el.style.height = "50vh";
          }}
        />
        <div
          className="cb-cinema-bar cb-cinema-bar-bottom"
          ref={(el) => {
            if (el) el.style.height = "50vh";
          }}
        />
        ...
        <div className="cb-tech-hud">
          ...
          <div className="cb-hud-text cb-hud-status">
            SYSTEM STATUS: OPTIMAL
          </div>
          <div className="cb-hud-text cb-hud-coords">
            30.7333° N, 76.7794° E
          </div>
        </div>

        <div
          className="cb-hero-content"
          ref={heroContentRef}
          style={{ opacity: 0 /* Reset to 0 for anim */ }}
        >
          {/* VARIABLE PROXIMITY TITLE (Restored) */}
          <div
            className="cb-hero-title-wrap"
            ref={titleWrapRef}
            style={{
              display: "flex",
          ...
          <p
            className="cb-hero-desc"
            ref={(el) => {
              if (el && introDone)
                gsap.to(el, { opacity: 1, duration: 1, delay: 0.5 });
            }}
          >
            // Forging the digital frontier of IEEE SBNU. Code Blue constitutes
            the elite technical division, engineering the future line by line.
          </p>
        </div>

        <div
          className="cb-scroll-cue"
          ref={scrollCueRef}
          style={{ opacity: 0 }}
        >
      ...
      <div className="cb-marquee">
        <div className="cb-marquee-track">
          {renderMarqueeItems(MARQUEE_TEXT_1)}
        </div>
      </div>
      ...
      <section className="cb-attitude">
        <div className="cb-attitude-wrap">
          <p
            className="cb-attitude-text"
            ref={attitudeTextRef}
            style={{ transform: "translateY(30px)", opacity: 0 }}
          >
            WE DON&apos;T FOLLOW <span className="em">TRENDS</span>. WE SET{" "}
            <span className="em">THEM</span>.
          </p>
          <p
            className="cb-attitude-sub"
            ref={attitudeSubRef}
            style={{ opacity: 0 }}
          >
            // THE TECHNICAL DIVISION OF IEEE SBNU — CODE BLUE 2026
          </p>
        </div>
      </section>
      ...
      <section className="cb-team-section" style={{ position: "relative" }}>
        <div className="cb-team-header">{/* REMOVED TITLE AS REQUESTED */}</div>
            ...
            >
              ...
              <div className="cb-layer-content">
                ...
                <div className="cb-card-info">
                  ...
                  <div className="cb-card-role">{leader.role}</div>{" "}
                  {/* Removed TextScramble */}
                  {/* BIO TITLE */}
                  <div
                    className="cb-bio-title"
                    style={{
                      color: "#5eb8ff",
                      fontSize: "0.75rem",
                      fontWeight: "700",
              ...
              <div className="cb-layer-holo">
                {leader.stats?.map((stat, i) => (
                  <div key={i} className="cb-stat-item">
                    {stat}
                  </div>
                ))}
            ...
            >
              ...
              <div className="cb-layer-content">
                ...
                <div className="cb-card-info">
                  ...
                  <div className="cb-card-role">{member.role}</div>{" "}
                  {/* Removed TextScramble */}
                  {/* BIO TITLE */}
                  <div
                    className="cb-bio-title"
      ...
      <section className="cb-outro">
        <p className="cb-outro-pre" ref={outroPreRef} style={{ opacity: 0 }}>
          SYSTEM DESIGNED & BUILT BY
        </p>
        <h2
          className="cb-outro-title"
          ref={outroTitleRef}
          style={{ opacity: 0 }}
        >
          CODE <span className="blue">BLUE</span>
        </h2>
        <div
          style={{
        ...
        >
          ...
          <span style={{ color: "#fff" }}>~</span> echo "Hello, World"
        </div>
        <p className="cb-outro-end" ref={outroEndRef} style={{ opacity: 0 }}>
          &lt; END TRANSMISSION &gt;
        </p>
      </section>
    </div>

...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  float hitIsY = 1.0;
    ...
    {
        float isY = float(i < 2);
        float pos = mix(-uPlaneH, uPlaneH, float(i)) * isY + mix(-uPlaneV, uPlaneV, float(i - 2)) * (1.0 - isY);
  ...
  if (uLineStyle > 0.5) {
    ...
    float edgeDistX = min(abs(hit.x - (-uPlaneV)), abs(hit.x - uPlaneV));
    float edgeDistY = min(abs(hit.y - (-uPlaneH)), abs(hit.y - uPlaneH));
...
export const GridScan = ({
  ...
  useEffect(() => {
    ...
    // Auto-compute plane spreads based on aspect ratio
    const computePlanes = (w, h) => {
      const aspect = w / h;
      if (aspect >= 1.2) {
        uniforms.uPlaneV.value = 0.5;
        uniforms.uPlaneH.value = 0.2;
      } else if (aspect >= 0.7) {
        uniforms.uPlaneV.value = 0.35;
        uniforms.uPlaneH.value = 0.25;
      } else {
        uniforms.uPlaneV.value = 0.25;
        uniforms.uPlaneH.value = 0.35;
      }
    };
    computePlanes(container.clientWidth, container.clientHeight);
  ...
  useEffect(() => {
    ...
    noiseIntensity, scanGlow, scanOpacity, scanDirection,
